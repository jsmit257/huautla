...
func (db *Conn) SelectAllIngredients(ctx context.Context, cid types.CID) ([]types.Ingredient, error) {
	...
	deferred, start, l := initVendorFuncs("SelectAllIngredients", db.logger, err, "nil", cid)
...
func (db *Conn) SelectIngredient(ctx context.Context, id types.UUID, cid types.CID) (types.Ingredient, error) {
	...
	deferred, start, l := initVendorFuncs("SelectIngredient", db.logger, err, id, cid)
	defer deferred(start, err, l)
...
func (db *Conn) InsertIngredient(ctx context.Context, i types.Ingredient, cid types.CID) (types.Ingredient, error) {
	...
	deferred, start, l := initVendorFuncs("InsertIngredient", db.logger, err, i.UUID, cid)
	defer deferred(start, err, l)
...
func (db *Conn) UpdateIngredient(ctx context.Context, id types.UUID, i types.Ingredient, cid types.CID) error {
	...
	deferred, start, l := initVendorFuncs("UpdateIngredient", db.logger, err, id, cid)
	defer deferred(start, err, l)
...
func (db *Conn) DeleteIngredient(ctx context.Context, id types.UUID, cid types.CID) error {
	...
	deferred, start, l := initVendorFuncs("DeleteIngredient", db.logger, err, id, cid)
	defer deferred(start, err, l)
	...
	result, err = db.ExecContext(ctx, db.sql["ingredient"]["delete"], id)
	if err != nil {
		return err
	} else if rows, err := result.RowsAffected(); err != nil {
		return err
		...
		return fmt.Errorf("ingredient could not be deleted: '%s'", id)
	}
